...
[`@formatjs/unplugin`](/docs/tooling/unplugin) is a universal build plugin that supports **Vite**, **Webpack**, **Rollup**, **esbuild**, **Rspack**, and **Rsbuild** with one package. It provides the same transformations as `babel-plugin-formatjs` without requiring Babel.
