...
A message must contain human language. Do not declare a message whose entire content is punctuation or a symbol, such as `—`, `•`, or `N/A`-style glyphs. It gives translators nothing to act on and adds noise to every locale's catalog. Render such glyphs as literal text or markup, and when a glyph is genuinely locale-sensitive — list separators or quotation marks — use `formatList` or CSS `quotes` instead of a message.
...
Do not hardcode a currency symbol or code around a placeholder. Symbol choice, placement, spacing, and grouping all vary by locale, and writing `$` or `USD` into the text freezes the English convention everywhere.

```tsx
// Avoid: '$' and 'USD' are frozen in every locale.
intl.formatMessage(
  {defaultMessage: 'Minimum ${min} USD, maximum ${max} USD'},
  {min, max}
)

// Prefer: the skeleton emits the right symbol and placement per locale.
intl.formatMessage(
  {
    defaultMessage:
...
If the currency code is only known at runtime, format the value with `formatNumber(value, {style: 'currency', currency})` and pass the result as the placeholder value instead of spelling the currency in the message.

The same applies to signs. A hardcoded `+` or `-` glued to a placeholder bypasses locale-aware sign placement and glyphs, such as locale-specific minus signs and accounting notation. Use the [sign-display](https://unicode-org.github.io/icu/userguide/format_parse/numbers/skeletons.html#sign-display) skeleton options instead.

```tsx
// Avoid: the '+' is attached to the number by hand.
intl.formatMessage(
  {defaultMessage: 'Subtotal (+{amount} credits)'},
  {amount}
)

// Prefer: sign display is part of number formatting.
intl.formatMessage(
