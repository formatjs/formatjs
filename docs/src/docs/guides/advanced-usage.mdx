...
<Admonition type="caution" title="Caching">
  Since this approach uses `AST` as the data source, changes to
  `@formatjs/icu-messageformat-parser`'s `AST` will require cache invalidation.
</Admonition>
<Admonition type="caution" title="Asset Size">
  `AST` is also slightly larger in size than regular `string` messages but can
