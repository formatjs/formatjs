...
<Admonition type="info" title="Future Changes">
  This `IntlMessageFormat` API may change to stay in sync with ECMA-402, but
  this package will follow [semver](http://semver.org/).
</Admonition>
...
<Admonition type="danger" title="placeholders">
  A value **must** be supplied for every argument in the message pattern the
...
Typed APIs require TypeScript 5.4 or newer.

Pass an argument contract as the class generic to check `format` and
`formatToParts`. Omitting the generic keeps the existing permissive API: TypeScript does not
infer required arguments or their types from the message literal. Missing values
can still cause a runtime formatting error.
...
message.format({count: 2}) // OK
message.format({count: 'two'}) // TypeScript error
message.formatToParts() // TypeScript error: missing count
```

The contract works with both message strings and parsed ASTs. The opt-in
[`formatjs/enforce-message-types`](/docs/tooling/linter#enforce-message-types)
rule generates and refreshes the generic for static strings using the ICU
parser. TypeScript itself does not verify that a handwritten contract matches
the message.

`MessageValue` describes an unformatted argument. `MessageTag` describes a
rich-text callback. For rich objects, supply the output type to `format<T>` or
`formatToParts<T>`:

```ts
import IntlMessageFormat, {type MessageTag} from 'intl-messageformat'

const rich = new IntlMessageFormat<{b: MessageTag}>('<b>Hello</b>', 'en')
const result = rich.format<{children: unknown[]}>({
  b: chunks => ({children: chunks}),
})
```

These contracts also back `@formatjs/intl` and React Intl typed descriptors.
Framework integrations supply their own rich-value types; the shared contract
has no React or message-descriptor dependency.

Unlike the framework methods, this class takes `Values` on
`IntlMessageFormat<Values>` and rich output on `format<RichOutput>` or
`formatToParts<RichOutput>`. See the
[core Intl](/docs/intl#typed-message-arguments) and
[React Intl](/docs/react-intl/api#typed-message-arguments) guides for
`formatMessage<Values, RichOutput>` and `$t<Values, RichOutput>`.

Contract properties can be optional, for example `{count?: number}` or
`{b?: MessageTag}`. Their types are checked when supplied. If every property is
optional, the values argument can be omitted; runtime ICU requirements still apply.
