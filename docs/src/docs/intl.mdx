...
To distinguish formatted messages from plain strings, add this to your app's
...
Text-only `intl.formatMessage`, `intl.$t`, and `TextMessageFormatter` calls now
return `FormattedText`, a string type that rejects unformatted text:
...
All framework integrations export the same `FormattedText` type. It still works
where a `string` is expected, but string operations generally lose the brand.
Fallback text also qualifies: this type doesn't guarantee translation or safety.

Enable this in apps, not reusable libraries: it affects the whole TypeScript
program. Runtime behavior, rich text, other formatters, `IntlMessageFormat.format()`,
and the standalone core `formatMessage` function are unchanged. Without opt-in,
text output stays `string`; `MessageTextOutput` reflects the selected mode.
