...
For TypeScript argument checks and generic defaults, see [Typed message arguments](#typed-message-arguments).
...
With no generics, a plain descriptor keeps the permissive API: TypeScript checks
the descriptor and supported value types, but does not infer required placeholder
names or their types from the ICU string. Missing values can still fail at runtime.
A typed helper descriptor carries its contract, so no call-site generic is needed.
...
Applications can require formatter-produced text at component or helper boundaries.
Add this declaration to a file included by the application's TypeScript project:
...
Text-returning overloads of `intl.formatMessage`, `intl.$t`, and
`TextMessageFormatter` then return `FormattedText` instead of `string`.

```ts
import {createIntl, type FormattedText} from '@formatjs/intl'

const intl = createIntl({locale: 'en'})
function pageTitle(title: FormattedText): string {
  return title
}

pageTitle(intl.formatMessage({id: 'home.title', defaultMessage: 'Home'}))
pageTitle('Home') // TypeScript error: plain strings lack the brand.
```

`FormattedText` is a type-only string brand defined by `intl-messageformat` and
re-exported by `@formatjs/intl`, `react-intl`, `react-intl/server`, `vue-intl`, and
`@formatjs/svelte-intl`. All integrations use the same brand. `MessageTextOutput`
is the conditional output type: `string` by default, `FormattedText` with opt-in.

The brand records provenance only. Fallback IDs, default messages, and raw
fallback text also qualify. It does not guarantee translation availability,
safe content, or approved interpolation inputs. Rich-text overloads, other
formatting APIs, `IntlMessageFormat.format()`, and the standalone core
`formatMessage` function retain their existing return types. Runtime behavior
is unchanged.

Augmentation affects the entire TypeScript program, including dependencies;
reusable libraries should not enable it for their consumers. Branded text remains
assignable to `string`, but string operations generally lose the brand. Inferred
variables cannot later receive plain strings, and formatter mocks must return
branded text. Annotate a variable as `string` when plain-string reassignment is
intended. Without opt-in, existing return types are unchanged.
