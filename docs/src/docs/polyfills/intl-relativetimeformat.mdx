...
- `Intl.NumberFormat` with `formatToParts` and support for the selected numbering system, or the [NumberFormat polyfill](intl-numberformat).

The `numberingSystem` option and the locale's `nu` extension support CLDR numeric
systems. For example, English with `numberingSystem: 'arab'` formats 1234.5 days as
`in ١٬٢٣٤٫٥ days`. Load the NumberFormat polyfill when the host lacks the requested
numbering system.
