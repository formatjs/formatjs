...
install the corresponding package. The old calendar locale paths are removed;
arithmetic-only `calendar-data/<calendar>.js` imports remain supported.
`add-all-calendars.js` explicitly loads all calendar arithmetic and tables;
localized calendar patterns still load separately. None of these optional
modules installs `globalThis.Temporal`.
