...
### **`onMsgDescriptorExtracted(filePath: string, occurrence: MessageDescriptorOccurrence)`**

Callback that receives every successfully extracted descriptor occurrence along with exact writable source ranges for static `id` and `defaultMessage` literals. Duplicate IDs are reported separately. Writable ranges are omitted for concatenations, tagged templates, and descriptors containing spreads. Descriptors that cannot be statically extracted are not reported.
