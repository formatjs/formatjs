...
| --------- | ------------ | -------- | ------------------------------------------------------------------ |
| `name`    | `string`     | Yes      | Target name                                                        |
| `srcs`    | `label_list` | Yes      | Source JSON files with extracted messages                          |
| `out`     | `string`     | Yes      | Output compiled JSON file                                          |
...
| -------------- | ------------ | -------- | ----------------------------------------------- |
| `name`         | `string`     | Yes      | Target name                                     |
| `translations` | `label_list` | Yes      | Translation files, with the source locale first |
...
formatjs_compile(
    name = "compile_dev",
    srcs = [":extract_dev"],
    out = "messages-compiled.json",
    ast = True,
)

formatjs_verify_test(
    name = "verify_dev",
    translations = [":extract_dev"] + glob(["translations/*.json"]),
)
```
...
formatjs_verify_test(
    name = "verify_all",
    translations = [":all_app_messages"] + glob(["translations/*.json"]),
)
```
...
Complete working examples live in the FormatJS repository:

- [examples/simple](https://github.com/formatjs/formatjs/tree/main/rules/formatjs/examples/simple) - Basic message extraction, compilation, and verification
- [examples/aggregate](https://github.com/formatjs/formatjs/tree/main/rules/formatjs/examples/aggregate) - Aggregating messages from multiple modules
...
3. Add custom components/functions using `additional_component_names` and `additional_function_names`
...
- [rules_formatjs source](https://github.com/formatjs/formatjs/tree/main/rules/formatjs)
