...
intl.$t<{b?: MessageTag; count?: number | bigint; extra?: MessageValue}>({
...
} = defineMessages<{count: {n: number | bigint}}>(
...
both the call generic and the annotation when messages change. Keys come from the actual catalog; readonly properties remain readonly.
