...
refreshes existing generated contracts. Use a TypeScript-capable ESLint parser.
JavaScript files receive inline-value checks but no generated types. Dynamic
message strings cannot supply a contract.

```ts
const message = defineMessage</* @formatjs-generated */ {
  count: number | bigint
}>(
  {
    id: 'items',
...
The helper's import module supplies these types through inline type imports.
...
```ts
intl.formatMessage</* @formatjs-generated */ {
  count: number | bigint
}>({defaultMessage: '{count, number}'}, {count: 2})
```

The formatter signature validates the descriptor and values. No assertion or
runtime wrapper is needed. Later fixes refresh only the generated first generic,
...
If `@typescript-eslint/consistent-type-imports` is enabled, allow inline type
imports with `disallowTypeAnnotations: false`. Handwritten contracts are checked
but never rewritten; descriptor assertions are left unchanged. ID-only
descriptors cannot supply an argument contract.

Direct formatter construction receives the generic without an extra runtime option:

```ts
const message = new IntlMessageFormat</* @formatjs-generated */ {
  count: number | bigint
}>('{count, number}', 'en')
...
Generated contracts carry the `@formatjs-generated` comment so later fixes can
refresh them. Handwritten generics are never overwritten. Typed declarations
with unsupported or dynamic source expressions receive a diagnostic instead of
a guessed contract. Spreads and computed properties are not generated.
