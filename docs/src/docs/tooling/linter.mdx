...
For an annotated `defineMessages` variable, autofix preserves the original
annotation and adds per-message `TypedMessageDescriptor` contracts:
...
export const messages: Record<'count', MessageDescriptor> & {
...
both the call generic and the annotation when messages change. Original finite
key constraints and readonly properties remain enforced.
...
`Record<string, MessageDescriptor>` still permits dynamic indexing, and widening
a selected message later still erases its contract.
