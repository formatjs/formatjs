...
Opt-in ICU argument contracts for TypeScript `defineMessage` and `defineMessages`
...
JavaScript files and untyped dynamic messages are left unchanged.
...
Inline descriptors get a type-only annotation; values and formatting options
stay in place:
...
intl.formatMessage(
  ...
  } satisfies import('react-intl').MessageDescriptor /* @formatjs-generated */ as import('react-intl').TypedMessageDescriptor<{
    count: number | bigint
  }>,
  {count: 2}
)
...
The `satisfies` check retains descriptor validation. The assertion adds the
phantom argument contract without a runtime wrapper or import. Message extraction
continues to see the inline descriptor. Missing or wrongly typed values become
TypeScript errors, and later fixes refresh the generated annotation.

For inline calls, `moduleSource` can be `@formatjs/intl`, `react-intl`, or
`react-intl/server`. By default the rule reuses the generated annotation's module,
then the first matching import in the file, then `@formatjs/intl`. Set
`moduleSource: 'react-intl'` when an application wrapper supplies the formatter
without a direct FormatJS import. The selected package must be available to the
TypeScript project. If `@typescript-eslint/consistent-type-imports` is enabled,
allow inline type imports with `disallowTypeAnnotations: false`. Handwritten formatter generics and descriptor assertions
are left unchanged. ID-only descriptors cannot supply an argument contract.
