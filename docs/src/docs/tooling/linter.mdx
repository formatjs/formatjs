...
Use `anything` to require `defaultMessage` while allowing variables and function calls:
...
Without an option, the rule only checks that `defaultMessage` is present.
Both `recommended` and `strict` set `literal`. ESLint preserves inherited options
when an override only changes severity, so `'error'` or `['error']` will not
clear `literal`. Set `anything` after the preset to override it. Setting `off`
...
`--fix` converts direct JSX children (including fragments) when the formatter
comes from a local `const` binding to React Intl's `useIntl()`. It supports
destructured message formatters, reuses unshadowed `FormattedMessage` imports,
and adds an import alias when needed to avoid name collisions. Message
properties and the values argument stay as JavaScript expressions.

Fixes require an inline descriptor containing only `id`, `defaultMessage`,
and `description`. Calls with options, optional chaining, type arguments,
descriptor spreads/computed keys/accessors, or comments remain report-only.
Unverified formatter bindings (including standalone `createIntl` instances)
also remain report-only because their configuration may differ from the React
provider. Existing `useIntl` calls and bindings are retained to preserve hook
ordering; unused bindings can be cleaned up separately.
...
Checks message values and keeps TypeScript argument types in sync with ICU
messages. In JavaScript and untyped calls, it checks inline values like
[`enforce-placeholders`](#enforce-placeholders). Typed calls also let TypeScript
check variables, missing arguments, and incorrect value types.

### Setup

Use a TypeScript-capable ESLint parser, enable the rule, and run `eslint --fix`:
...
The rule is not included in the recommended or strict presets. Keep it enabled:
TypeScript checks the generated types, but only the linter checks that they still
match your message text.
...
intl.formatMessage(items, {count: 2}) // OK
intl.formatMessage(items, {count: 'two'}) // Type error
intl.formatMessage(items) // Type error: count is required
```

### What autofix supports

- `defineMessage` and `defineMessages` from `@formatjs/intl`, `react-intl`, or
  `react-intl/server`.
- Inline `formatMessage`, `$t`, and `$formatMessage` calls.
- `new IntlMessageFormat` from `intl-messageformat`.
- Catalog entries that reference existing messages, including identifier and
  enum-member keys. References use `MessageValuesOf<typeof message>`.
- Local `const` descriptors used only by message helpers and formatter calls.

Autofix updates the first generic, including handwritten types, and preserves
an optional second generic. It adds type imports as needed. Helpers default to
no arguments, so empty messages need no `<{}>` or `{typed: true}` option.
Mixed catalogs and inline calls that need an explicit empty contract use
`NoMessageValues`, imported from the matching FormatJS package. This type is
`Record<never, never>`, so generated contracts work with
`@typescript-eslint/no-empty-object-type` without disabling the rule.
...
const messages = defineMessages<{
  readonly hello: NoMessageValues
  readonly welcome: {readonly name: MessageValue}
}>({
  hello: {defaultMessage: 'Hello'},
  welcome: {defaultMessage: 'Welcome, {name}'},
})
```
...
| ICU argument      | Type                                      |
| ----------------- | ----------------------------------------- |
| Number or plural  | `number \| bigint`                        |
| Date or time      | `number \| Date`                          |
| Select            | `string`                                  |
| Plain placeholder | `MessageValue`                            |
| Rich-text tag     | `MessageTag` (a callback when formatting) |

Types include arguments from every ICU branch. Incompatible uses of the same
argument produce a diagnostic instead of a fix. Generated properties are readonly;
this does not freeze objects at runtime.

### Options

- **`generateTypes`** (default `false`): add types to untyped TypeScript messages.
  Existing typed messages are checked and refreshed even when this is off.
- **`ignoreList`**: argument names supplied by your wrapper. Inline checks ignore
  them; generated types make them optional and allow listed extra keys. Values
  still need the right types when supplied. This option does not provide values
  at runtime.
- **`moduleSource`**: package to import types from for inline formatter calls.
  Choose `@formatjs/intl`, `react-intl`, or `react-intl/server`. By default, the
  rule uses the first matching import, falling back to `@formatjs/intl`. Set this
  explicitly when your formatter comes from an application wrapper.

### Explicit annotations and dynamic keys

Autofix replaces broad `MessageDescriptor` annotations with
`TypedMessageDescriptor` types, including per-message types for catalogs. It
supports local aliases, imported descriptor aliases, `Record` types, property
maps, and `Readonly` wrappers. These annotations work with `isolatedDeclarations`.

**Catalog keys become exact:** broad index signatures and optional entries are
replaced with the messages actually present. Narrow dynamic keys to
`keyof typeof messages` before indexing. A selected message needs values valid
for every message it could refer to.

### Limits

Type generation needs static message text. It cannot derive types from ID-only
messages, dynamic strings, spreads, or arbitrary computed properties. Constructor
fixes support a static `ignoreTag` option, but not parsed AST input or dynamic
parser options. Unsupported typed declarations produce diagnostics rather than
guessed types; descriptor assertions are left unchanged.

Keep generated types when passing messages through wrappers. Widening to
`MessageDescriptor` or `any` loses the checks. This rule generates no JavaScript
types and does not validate translated catalogs.
