...
is preserved. Empty contracts on reusable helpers and calls supplying values
remain useful for checking later calls or rejecting extra values.
...
metadata and permissive formatting. Existing `{typed: true}` calls remain valid. Both `formatMessage` and
