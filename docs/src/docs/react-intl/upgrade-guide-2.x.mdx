...
<Admonition type="info">
  The `locale` prop is **singular**, required, and only accepts a string value.
  This is a simplification of the plural `locales` prop used by the `IntlMixin`.
</Admonition>
...
<Admonition type="info">
  This upgrade guide will focus on using Message Descriptors that only contain
  an `id` property.
</Admonition>
...
<Admonition type="info">
  Message ids can still contain `"."`s, so the ids themselves remain the same,
  it's only the `messages` object structure that needs to change.
</Admonition>
