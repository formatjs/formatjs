...
`FormattedMessage` also accepts a selection between an untyped descriptor and
a typed descriptor with no ICU arguments when `values` is omitted. Required
arguments in a typed branch remain checked.

For typed descriptors, a `MessageTag` contract infers the callback parameter as
`React.ReactNode[]`; no callback annotation is needed:

```tsx
import {defineMessage, FormattedMessage, type MessageTag} from 'react-intl'

const greeting = defineMessage<{b: MessageTag}>(
  {id: 'greeting', defaultMessage: 'Hello <b>world</b>'},
  {typed: true}
)
