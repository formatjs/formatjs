...
Opt into [branded message text](/docs/intl#branded-message-text) to get
`FormattedText` from text-only `formatMessage`, `$t`, and `TextMessageFormatter`
calls. Both React entry points export this type and `MessageTextOutput`.
Rich text and `<FormattedMessage>` are unchanged.
