...
<Admonition type="danger" title="multiple language support">
  ...
  [`formatMessage`](#formatmessage) instead.
</Admonition>
...
take ICU argument types first. The optional second generic defaults to
`React.ReactNode`; omit it for normal React rich-text output.

`$t` shares all of `formatMessage`'s overloads, including contextual typing for
rich-text callbacks in calls without generics.

```tsx
import {type MessageTag, type MessageValue} from 'react-intl'

intl.$t<{name: MessageValue; b: MessageTag}, React.ReactNode>(
  {defaultMessage: '<b>{name}</b>'},
  {name: 'Ada', b: chunks => <b>{chunks}</b>}
)
```

`MessageValue` represents an unformatted placeholder; `MessageTag` represents a
tag callback. Use specific types such as `number | bigint` for numeric arguments.

Without generics, a plain descriptor stays permissive: TypeScript does not parse
the ICU string to require arguments or check each placeholder's type.
For example, `intl.$t({defaultMessage: '{count, number}'})` still compiles,
although missing values can cause a runtime formatting error.

A descriptor from `defineMessage<Values>(descriptor)` or
`defineMessages<Contracts>(messages)` carries its contract into
calls without generics. Helpers without generics carry an empty contract; later
formatting calls reject extra values. `TypedMessageDescriptor` defaults to that
same empty contract. For messages with arguments, supply the contract or enable
`formatjs/enforce-message-types` with `generateTypes: true`.
Handwritten contracts must match the message; TypeScript does not verify that
match. Runtime formatting is unchanged. The same rules apply to formatters and
typed helpers from `react-intl/server`.

See the [12.x upgrade guide](/docs/react-intl/upgrade-guide-12.x) for migration
from the previous generic positions.
...
intl.formatMessage(messages.greeting, {name: 'Eric'})
```
...
intl.formatMessage(messages.greeting, {name: <b>Eric</b>})
```
...
### Sharing text-only formatters

Use `TextMessageFormatter` for helpers that accept either a core or React
formatter and produce text, such as page titles and metadata. It is exported
from `@formatjs/intl`, `react-intl`, and `react-intl/server`.

```ts
import type {TextMessageFormatter} from '@formatjs/intl'

function pageTitle(format: TextMessageFormatter, count: number): string {
  return format<{count: number}>(
    {id: 'page.items', defaultMessage: '{count, number} items'},
    {count}
  )
}

pageTitle(coreIntl.formatMessage, 2)
pageTitle(reactIntl.$t, 2)
```

This type preserves explicit argument contracts, typed descriptors, and registered
message IDs. Rich-text callbacks receive `string[]` and must return strings;
React elements are not accepted. It adds no runtime wrapper and does not change
the full formatter types used for rendering rich content.

### Branded message text

Enable [branded message text](/docs/intl#branded-message-text) through
`FormatjsIntl.MessageFormatting` to return `FormattedText` from text-returning
`intl.formatMessage`, `intl.$t`, and `TextMessageFormatter` overloads. Both
`react-intl` and `react-intl/server` re-export `FormattedText` and
`MessageTextOutput`. The brand is shared with the core and other frameworks;
React rich-text output and `<FormattedMessage>` remain unchanged.
