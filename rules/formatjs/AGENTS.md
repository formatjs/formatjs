...
Follow [docs/releasing.md](docs/releasing.md). Release workflows live at the
FormatJS repository root.
...
- Release Please tags trigger `rules-formatjs-release.yml` automatically.
  Dispatch it only to recover an existing tag. Use
  `rules-formatjs-publish.yml` only to retry BCR publication for an existing
  release.
- Monitor the workflow through the GitHub release and BCR handoff. Tag creation
  alone is not completion.
- If the BCR push rejects the token, rotate the secret and retry
  `rules-formatjs-publish.yml` with the existing tag. Do not rerun Release
