...
```starlark
bazel_dep(name = "rules_formatjs", version = "0.11.0")
```

For local development against the FormatJS checkout:
...
formatjs_verify_test(
    ...
    translations = [
        ":messages_extracted",  # Base language
...
formatjs_verify_test(
    name = "verify_es",
    translations = [":extract_en", "translations/es.json"],
)
```
...
- `name` (required): Target name
- `srcs` (required): Source JSON files with extracted messages
- `out` (required): Output compiled JSON file
...
- `name` (required): Target name
- `translations` (required): Translation files, with the source locale first
