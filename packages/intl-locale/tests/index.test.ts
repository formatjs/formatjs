...
describe('intl-locale', () => {
  ...
  describe('new locale info proposal properties', () => {
    it('ar', () => {
      ...
      expect(locale.calendars).toEqual(['gregory', 'coptic', 'islamic', 'islamicc', 'islamic-tbla'])
    ...
    it('ar-EG', () => {
      ...
      expect(locale.hourCycles).toEqual(['h12', 'h23'])
      expect(locale.numberingSystems).toEqual(['arab'])
      expect(JSON.stringify(locale.timeZones)).toMatch(JSON.stringify(['Africa/Cairo']))
    })

    it('ar-SA', () => {
      const locale = new Locale('ar-SA')
      expect(locale.calendars).toEqual(['islamic-umalqura', 'gregory', 'islamic', 'islamic-rgsa'])
      expect(locale.collations).toEqual(['compat', 'emoji', 'eor'])
      expect(locale.hourCycles).toEqual(['h12', 'h23'])
      expect(locale.numberingSystems).toEqual(['arab'])
      expect(JSON.stringify(locale.timeZones)).toMatch(JSON.stringify(['Asia/Riyadh']))
    })
