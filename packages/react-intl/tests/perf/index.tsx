...
async function run() {
  ...
  bench.add('<div>', function () {
    ReactDOMServer.renderToString(<div />)
  })

  bench.add('100 x <div/>', function () {
    const divs = []
  ...
  bench.add('100 x <FormattedNumber>', function () {
  ...
  bench.add('100 x <FormattedDate>', function () {
    let now = Date.now()
    let formattedDates = []
    for (let i = 0, len = 100; i < len; i += 1) {
  ...
  bench.add('100 x <FormattedMessage>', function () {
  ...
  bench.add('100 x <FormattedMessage> with placeholder', function () {
    let messages: Record<number, string> = {}
    let formattedMessages = []
    for (let i = 0, len = 100; i < len; i += 1) {
      const varName = `var${i}`
  ...
  bench.add(
    '100 x <FormattedMessage> with placeholder in AST form',
    function () {
      let formattedMessages = []
  ...
  bench.add('100 x <FormattedMessage> with placeholder, cached', function () {
    let messages: Record<number, string> = {}
    let formattedMessages = []
    for (let i = 0, len = 100; i < len; i += 1) {
  ...
  // bench.add('100 x <FormattedRelative>', function() {
