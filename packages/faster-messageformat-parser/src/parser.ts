...
export class Parser {
  ...
  private tryParseQuote(parentArgType: ArgType): string | null {
    ...
    switch (this.peek()) {
      ...
      case 125:
        break
      // @ts-expect-error TODO: No fallthrough in strict mode
      case 35: // '#'
    ...
    switch (argType) {
      ...
      case 'time': {
        ...
        if (this.bumpIf(',')) {
          ...
          // @ts-ignore TODO: trimEnd is too new
        ...
        if (styleAndLocation?.style.startsWith('::')) {
          ...
          // @ts-ignore TODO: trimStart is too new
        ...
        if (argType === 'select') {
          return {
            val: {
              type: TYPE.select,
              value,
              // @ts-expect-error TODO: fromEntries is too new
