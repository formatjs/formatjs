...
```
Source .ts/.tsx files
  ├─ formatjs_package_json(:package_json) → generated package metadata
...
Package manifests under `packages/**` are generated by Bazel and live in the
output tree, not in source. Static package fields live in each package's
