...
```
Source .ts/.tsx files
  ├─ formatjs_package_json(:generated_package_json) → package.generated.json
...
Package manifests under published `packages/**` are checked in so pnpm sees
them as workspace projects. Static package fields still live in each package's
`formatjs_library(package_name = ..., version = ..., ...)` or
`generate_package_json(package_name = ..., version = ..., ...)` rule, while
dependency sections come from Bazel npm labels. Bazel generates
`package.generated.json` and `package_json_sync` keeps the checked-in
`package.json` in sync. `:package_json` is a `copy_to_bin` wrapper around that
source file for cross-package Bazel consumers. Manifest ordering comes from
`tools/package_json_policy.bzl`; workspace dependencies use `workspace:*`,
external dependency ranges default to the major versions declared by the root
`package.json`, and package-local BUILD rules pass explicit overrides for
preserved compatibility ranges such as peer-dependency windows.
Bundling is done per `entry_points` with Rolldown and `rolldown-plugin-dts`.
Standalone published package manifests follow the same source-sync pattern. The
root `dist_package_json_test` builds `:dist` and compares each distributed
package manifest against the corresponding `:package_json` wrapper. The
`//:release_please_npm_workspace_graph` target reads those package manifests for
all release-managed npm packages, including native packages assembled by
...
bumps. Root `package.json` lists published packages as `workspace:*` dev
dependencies, package-local manifests define their dependency graph, and
their first-party dependencies also use `workspace:*`. `npm_link_all_packages()`
turns the resulting lockfile entries into root `//:node_modules/<workspace>`
labels.
...
- Package BUILD files pass static manifest fields through package field attrs
  ...
  versions come from root `package.json`: workspace dependencies use
  `workspace:*`, while external dependencies default to major-only ranges.
  `package_dependency_version_overrides` preserves intentional compatibility
  ranges. Manifest field ordering comes from `tools/package_json_policy.bzl`.
