...
`prerelease: false` with `versioning: prerelease` retained for stable promotion.
The runner registers a release-please 17.6 prerelease fix, preserving RC suffixes
when prerelease mode is enabled while delegating stable promotion normally.
`publishNpmPackages` derives the npm dist-tag from each version: stable versions
use `latest`, RCs use `rc`, and other prereleases use `next`. Publishing always
passes the tag explicitly. Review generated stable versions and dependency
updates before merging the release PR; do not retag RC artifacts.
